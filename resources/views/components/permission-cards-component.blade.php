    ...
    <div class="card">
        <div class="card-body">
            <div class="d-flex justify-content-between">
                <span>{{$permission_each->users->count()}} Users</span>
                <ul class="list-unstyled d-flex align-items-center avatar-group mb-0">
                    @foreach ($permission_each->users->take(5) as $each_user)
                    <li data-bs-toggle="tooltip" data-popup="tooltip-custom" data-bs-placement="top" title="{{$each_user->name}}" class="avatar avatar-sm pull-up">
                        <img class="rounded-circle" src="{{asset('assets/images/Profile')}}/{{$each_user->UserAttributes->image}}" alt="user" />
                    </li>
                    @endforeach
                </ul>
            </div>
            <div class="d-flex justify-content-between align-items-end mt-1 pt-25">
                <div class="role-heading">
                    ...
                    <a href="javascript:;" class="role-edit-modal" onclick="edit_permission({{$permission_each->id}})">
                        <small class="fw-bolder">Edit Users </small>
                    </a>
                </div>  
